...
/* ===== V1.7.5 แจ้งเตือนสต็อกใกล้หมด รวมใน kitchen.js ===== */
(() => {
  ...
  if (!panel || !summary || !list) {
    console.warn("ไม่พบส่วนแจ้งเตือนสต็อกใน kitchen.html");
    return;
  }
  ...
  async function loadLowStockAlerts() {
    ...
    const { data, error } = await sb
      .from("stock_items")
      .select("id,name,unit,quantity,minimum_quantity,category,is_active,sort_order")
      .eq("is_active", true)
    ...
    if (error) {
      ...
      list.innerHTML =
        `<div class="low-stock-alert-error">${error.message}</div>`;
      return;
    }
      ...
      .map(item => `
        <article class="low-stock-alert-item ${item.status}">
          ...
          <div class="low-stock-alert-qty">
            <b>${formatStockNumber(item.quantity)} ${item.unit}</b>
            <small>ขั้นต่ำ ${formatStockNumber(item.minimum_quantity)} ${item.unit}</small>
          </div>
        </article>
      `)
      .join("");
